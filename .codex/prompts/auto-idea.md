...
그리고 아래 `Visual Brief`를 함께 작성합니다:
...
`## Clarification Ledger`, Intent Brief, `## Visual Brief`, `## Outcome Lock`, `## Evolution Ideas`, 미해결 가정, debate focus를 BS 파일에 함께 기록합니다.

Plan Handoff mapping: `answered` rows feed requirements/scope/acceptance seeds, `assumed` rows feed risks/acceptance assumptions/validation experiments/reviewer focus, `deferred` rows feed research/open questions unless they block the Outcome Lock, and `scope_boundary` feeds explicit non-goals. `Evolution Ideas` are optional improvements and must not auto-create follow-up or sibling SPECs.

응답은 Visual Brief의 핵심 플로우차트 또는 wireframe 요지를 포함해 사용자에게 정리합니다.
